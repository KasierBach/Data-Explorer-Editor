...
export class AiChatController {
    ...
    @Post()
    createChat(@Request() req: any, @Body() dto: CreateAiChatDto) {
        return this.aiChatService.createChat(req.user.id, dto);
    }

    @Patch(':id')
    updateChat(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateAiChatDto) {
        return this.aiChatService.updateChat(req.user.id, id, dto);
    }

    @Delete(':id')
    deleteChat(@Request() req: any, @Param('id') id: string) {
        return this.aiChatService.deleteChat(req.user.id, id);
    }

    @Post(':id/messages')
    addMessage(@Request() req: any, @Param('id') id: string, @Body() dto: AddMessageDto) {
