...
export class AiProviderRunnerService {
  ...
  async runOpenAiCompatible(
    ...
    const needsSearch =
      /\b(search|news|latest|current|today|hôm nay|mới nhất|tin tức)\b/i.test(
        params.prompt,
      );
    ...
    try {
      ...
      if (
        ...
        requestTimeout = this.createAbortController(
          `${plan.provider} request (retry) (${modelToUse})`,
        );
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(
          `${plan.provider} error (${response.status}): ${error.error?.message || response.statusText}`,
        );
      }
  ...
  async *streamGemini(
    ...
    const result = await this.withTimeout(
      model.generateContentStream({
        contents: this.promptBuilder.buildGeminiContents(
          params.prompt,
          params.context,
          params.history,
          params.image,
  ...
  async *streamOpenAiCompatible(
    ...
    try {
      ...
      if (
        ...
        requestTimeout.clear();
        requestTimeout = this.createAbortController(
          `${plan.provider} stream request (retry) (${modelToUse})`,
        );
        response = await fetch(`${plan.baseUrl}/chat/completions`, {
          method: 'POST',
          headers: this.getOpenAiCompatibleHeaders(plan),
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(
          `${plan.provider} Stream API error [${response.status}]: ${JSON.stringify(error)}`,
        );
      }
