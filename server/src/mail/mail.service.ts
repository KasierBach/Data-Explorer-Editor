...
export class MailService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!this.apiKey || !this.senderEmail) {
      this.logger.warn(
        'MAIL_USER or MAIL_PASS (API Key) not set. Emails will be logged to console only.',
      );
    } else {
      const maskedKey =
      ...
      this.logger.log(
        `Mail service initialized using Brevo REST API. Sender: ${this.senderEmail}, Key: ${maskedKey} (Length: ${this.apiKey.length})`,
      );
    }
  }
  ...
  private async send(to: string, subject: string, html: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to send email to ${to} via API: ${error.message}`,
      );
      throw error;
    }
  }
