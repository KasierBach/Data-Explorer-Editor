  ...
  progress(
    ...
    if (payload.jobId !== jobId) {
      throw new UnauthorizedException(
        'Migration progress ticket does not match this job.',
      );
    }

    return new Observable<MessageEvent>((observer) => {
      this.migrationService
        .getPublicJob(jobId, payload.sub)
        .then((initialJob) => {
      ...
      const cleanup = () => {
        ...
        this.migrationService.eventEmitter.removeListener(
          `migration-${jobId}`,
          listener,
        );
      };
