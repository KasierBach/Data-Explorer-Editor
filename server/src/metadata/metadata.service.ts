...
export class MetadataService {
  ...
  private async getCacheKey(
    ...
    return this.freshnessService.buildKey(prefix, [connectionId], [identifier]);
  }
  ...
  private async getConnectionContext(
    connectionId: string,
    userId: string,
    database?: string,
  ...
  private async _getHierarchyUncached(
    connectionId: string,
    parentId: string | null,
    userId: string,
  ) {
    ...
    if (parentId.includes('.folder:')) {
      const dbPool = await this.connectionsService.getPool(
        connectionId,
        parsed.dbName,
        userId,
      );
      ...
      if (parentId.endsWith('folder:tables'))
        return strategy.getTables(dbPool, schema, parsed.dbName);
      if (parentId.endsWith('folder:views'))
        return strategy.getViews(dbPool, schema, parsed.dbName);
      if (parentId.endsWith('folder:functions'))
        return strategy.getFunctions(dbPool, schema, parsed.dbName);
    ...
    if (
      parentId.includes('schema:') &&
      !parentId.includes('.table:') &&
      !parentId.includes('.view:') &&
      !parentId.includes('.func:') &&
      !parentId.includes('.collection:')
    ) {
      if (connection.type === 'mongodb' || connection.type === 'mongodb+srv') {
        return strategy.getTables(
          pool,
    ...
    if (
      ...
      const folders = [
        ...
        {
          id: `${parentId}.folder:constraints`,
          name: 'Constraints',
          type: 'folder',
          parentId,
          hasChildren: true,
    ...
    if (parentId.includes('.func:')) {
      ...
      const schema =
        parsed.schemaName || (connection.type === 'mssql' ? 'dbo' : 'public');
  ...
  async getDatabases(connectionId: string, userId: string) {
    ...
    return this.withCache(
      cacheKey,
      async () => {
        const { pool, strategy } = await this.getConnectionContext(
          connectionId,
          userId,
        );
  ...
  async getColumns(connectionId: string, tableId: string, userId: string) {
    ...
    return this.withCache(
      cacheKey,
      async () => {
        const { connection, strategy } = await this.getConnectionContext(
          connectionId,
          userId,
        );
