...
export function sanitizeNoSql(obj: any, depth = 0): any {
  if (depth > MAX_DEPTH) {
    throw new ForbiddenException('Query quá sâu hoặc chứa vòng lặp (NoSQL security).');
  }
...
export function validateNoSqlPayload(payload: any) {
  ...
  if (!payload.action || !allowedActions.includes(payload.action)) {
    throw new ForbiddenException(`Hành động NoSQL không hợp lệ: ${payload.action}`);
  }

  if (!payload.collection || typeof payload.collection !== 'string') {
    throw new ForbiddenException('Collection name is required and must be a string.');
  }
}
