...
export class ErdWorkspacesController {
  ...
  @Get()
  findAll(@Req() req: any, @Query('connectionId') connectionId?: string) {
  ...
  @Post()
  create(@Body() dto: CreateErdWorkspaceDto, @Req() req: any) {
    return this.erdWorkspacesService.create(dto, req.user.id);
  }

  @Patch(':id')
  update(@Param('id') id: string, @Body() dto: UpdateErdWorkspaceDto, @Req() req: any) {
