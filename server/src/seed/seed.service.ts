  ...
  async seedAdmin() {
    ...
    if (userCount === 0) {
      ...
      if (!adminEmail || !adminPassword) {
        ...
        if (!adminEmail) {
          this.logger.error(
            'CRITICAL: Cannot seed admin without ADMIN_EMAIL. Skipping seed.',
          );
          return;
        }
        ...
        this.logger.warn(
          `⚠️  Using GENERATED password for ${adminEmail}: ${adminPassword}`,
        );
        this.logger.warn(
          '⚠️  Please change this password immediately and configure your environment variables.',
        );
      }
