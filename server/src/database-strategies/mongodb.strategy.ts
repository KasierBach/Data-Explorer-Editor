...
import {
  sanitizeNoSql,
  validateNoSqlPayload,
} from '../common/utils/nosql-sanitizer.util';
...
const inferType = (value: unknown): string => {
  ...
  if (typeof value === 'object') {
    if ((value as { _bsontype?: string })._bsontype === 'ObjectId')
      return 'objectId';
    return 'object';
  }
  return typeof value;
...
export class MongoDbStrategy implements IDatabaseStrategy {
  ...
  async createPool(
    ...
    if (!uri) {
      ...
      const protocol =
        connectionConfig.type === 'mongodb+srv' ? 'mongodb+srv' : 'mongodb';
    ...
    this.logger.log(
      `[MongoDbStrategy] Connecting to: ${uri.replace(/\/\/[^@]+@/, '//***:***@')}`,
    );
  ...
  async executeQuery(
    ...
    try {
      ...
      throw new Error(
        `Invalid MongoDB query payload (must be valid JSON). Received: ${queryPayloadStr}`,
      );
    }
    ...
    const documents = Array.isArray(payload.documents)
      ? payload.documents.map((d) => sanitizeNoSql(d))
      : [];
  ...
  async importData(
    client: MongoClient,
    params: { schema: string; table: string; data: any[] },
  ): Promise<{ success: boolean; rowCount: number }> {
    const db = client.db();
    const col = db.collection(params.table);
  ...
  async getDatabases(client: MongoClient): Promise<TreeNodeResult[]> {
    try {
      ...
      this.logger.warn(
        '[MongoDbStrategy] listDatabases failed, falling back to current db:',
  ...
  async getRelationships(
    ...
    try {
            ...
            for (const doc of docs) {
              for (const key of Object.keys(doc)) {
                ...
                if (
                  ...
                  const targetMatch = collectionNames.find(
                    (c) =>
                      c.toLowerCase() === prefix ||
                      c.toLowerCase() === prefix + 's' ||
                      c.toLowerCase() === prefix + 'es',
                  );
      ...
      this.logger.warn(
        '[MongoDbStrategy] Failed to infer MongoDB relationships',
  ...
  async seedData(client: MongoClient): Promise<QueryResult> {
    ...
    if (existingProducts === 0) {
      await productsCol.insertMany([
        ...
        { name: 'Keyboard', price: 50.0, stock: 50 },
      ]);
    }

    return {
      rows: [{ message: 'Seed data inserted successfully' }],
      columns: ['message'],
      rowCount: 1,
    };
  }
