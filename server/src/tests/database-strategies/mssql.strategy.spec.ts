...
describe('MssqlStrategy', () => {
    ...
    describe('createPool', () => {
        it('should create pool with timeouts and idle config', async () => {
            ...
            expect(mssql.ConnectionPool).toHaveBeenCalledWith(expect.objectContaining({
                server: '127.0.0.1',
                user: 'sa',
    ...
    describe('executeQuery (OOM Protections)', () => {
        it('should automatically inject TOP 50000 into a bare SELECT query to prevent OOM', async () => {
            ...
            expect(mockRequest.query).toHaveBeenCalledWith('SELECT TOP 50000 * FROM big_table');
        });
        ...
        it('should not inject TOP if query already has a TOP clause', async () => {
            ...
            expect(mockRequest.query).toHaveBeenCalledWith('SELECT TOP 10 * FROM big_table');
        });

        it('should not inject TOP into non-SELECT queries (e.g. UPDATE)', async () => {
            mockRequest.query.mockResolvedValue({ recordset: undefined, rowsAffected: [5] });
            
            const result = await strategy.executeQuery(mockPool, "UPDATE users SET active = 1");

            expect(mockRequest.query).toHaveBeenCalledWith("UPDATE users SET active = 1");
        ...
        it('should slice the result array to 50000 max exactly as a secondary guard', async () => {
            ...
            mockRequest.query.mockResolvedValue({ recordset: massiveArray, rowsAffected: [] });
