    ...
    releases: [
      {
        ...
        title: pickLocalizedText(
          currentLang,
          'CockroachDB & MariaDB thực chiến, kiểm thử tấn công & nền web hardened',
          'Production-grade CockroachDB & MariaDB, Adversarial Testing & a Hardened Web Platform',
        ),
        ...
        features: [
          feature(
            currentLang,
            'database',
            'CockroachDB và MariaDB nâng cấp lên production-ready',
            'CockroachDB and MariaDB promoted to production-ready',
            'CockroachDB có riêng một strategy kế thừa Postgres với default port 26257 và metrics qua crdb_internal thay vì SQL chỉ có trên PostgreSQL; MariaDB có đủ views, functions, tham số hàm và chặn cập nhật rỗng trước khi sinh SQL sai cú pháp.',
            'CockroachDB now gets a dedicated Postgres-derived strategy with the 26257 default port and crdb_internal-based metrics instead of PostgreSQL-only SQL; MariaDB gains views, functions, function parameters, and a guard that blocks empty updates before invalid SQL is generated.',
          ),
          feature(
            currentLang,
            'settings',
            'TLS contract dùng chung cho MySQL/MariaDB và Redis',
            'Shared TLS contract for MySQL/MariaDB and Redis',
            'Form kết nối có toggle TLS riêng cho MySQL và MariaDB để tắt SSL cho server không hỗ trợ; kết nối Redis cũng được lưu flag TLS an toàn, kèm migration backfill giữ nguyên hành vi auto-TLS hiện tại.',
            'The connection form now has a TLS toggle for MySQL and MariaDB to disable SSL for servers that reject handshakes; Redis connections persist a proper TLS flag too, with a backfill migration preserving existing auto-TLS behavior.',
          ),
          feature(
            currentLang,
            'shieldCheck',
            'Bộ pentest đối kháng cho guard SQL/NoSQL và HTTP layer',
            'Adversarial pentest suites for SQL/NoSQL guards and the HTTP layer',
            'Thêm các suite tấn công mới mô phỏng bypass qua identifier smuggling, IPv4-mapped IPv6, CSRF header spoofing, validateHost fail-closed cho host rỗng, cùng fuzzing property-based cho mọi đầu vào.',
            'New attack suites simulate bypasses via identifier smuggling, IPv4-mapped IPv6, and CSRF header spoofing; validateHost is now fail-closed for empty hosts, with property-based fuzzing covering all inputs.',
          ),
          feature(
            currentLang,
            'zap',
            'Nền web hardened: PWA, sitemap, security.txt và e2e billing',
            'Hardened web platform: PWA, sitemap, security.txt, and billing e2e',
            'Thêm PWA manifest, sitemap với lastmod động, security.txt cho responsible disclosure, html lang đồng bộ ngôn ngữ app, và luồng e2e cho trang return sau thanh toán MoMo/ZaloPay.',
            'Added a PWA manifest, a sitemap with dynamic lastmod, a security.txt for responsible disclosure, an html lang synced to the app language, and an e2e flow for the MoMo/ZaloPay billing return page.',
          ),
          feature(
            currentLang,
            'globe',
            'Tìm kiếm sidebar NoSQL theo index và hiển thị độ trễ query',
            'Indexed NoSQL sidebar search and query duration display',
            'Sidebar NoSQL điều hướng qua chỉ mục search mới, search service được tinh chỉnh lại, và kết quả query hiển thị thời lượng thực thi (durationMs) để đo hiệu năng trực tiếp.',
            'The NoSQL sidebar navigates through a new search index, the search service was refined, and query results now show execution duration (durationMs) for direct performance visibility.',
          ),
        ],
      },
      {
        ...
        title: pickLocalizedText(
          currentLang,
          'Observability, điều khiển query & supply-chain CI',
          'Observability, Query Control & Supply-chain CI',
        ),
        features: [
          feature(
            currentLang,
            'zap',
            'Pino structured logging với correlation ID theo request',
            'Pino structured logging with per-request correlation IDs',
            'Production chạy pino structured logging kèm correlation ID cho từng request để truy vết lỗi xuyên suốt pipeline; môi trường dev vẫn giữ Nest default logger quen thuộc.',
            'Production runs pino structured logging with correlation IDs on every request for end-to-end tracing; development keeps the familiar Nest default logger.',
          ),
          feature(
            currentLang,
            'code2',
            'Hủy query đang chạy & panel Active Queries',
            'Running-query cancellation & the Active Queries panel',
            'Mỗi engine có cơ chế hủy riêng (pg_cancel_backend, KILL QUERY) nên việc hủy diễn ra ngay trên server; panel Active Queries liệt kê query đang chạy và cho hủy từng cái trực tiếp.',
            'Each engine has its own cancellation path (pg_cancel_backend, KILL QUERY) so cancels happen on the server; the Active Queries panel lists running queries and cancels them individually.',
          ),
          feature(
            currentLang,
            'sparkles',
            'AI Explain cache theo SQL & follow-up prompts',
            'SQL-keyed AI Explain cache & follow-up prompts',
            'Lời giải thích truy vấn bằng AI được cache theo nội dung câu SQL nên hỏi lại cùng query không tốn thêm chi phí; sau mỗi giải thích có thể đặt câu hỏi follow-up ngay trong luồng.',
            'AI query explanations are cached by SQL content so re-asking the same query costs nothing extra; each explanation supports inline follow-up questions.',
          ),
          feature(
            currentLang,
            'shieldCheck',
            'CI supply-chain: Trivy, SBOM, pin image & vá CVE Docker',
            'Supply-chain CI: Trivy, SBOM, pinned images & Docker CVE patches',
            'Pipeline containers thêm Trivy vulnerability scanning, sinh SBOM, pin base image; image runtime được strip npm toolchain và nâng cấp OS package để vá CVE-2026-73566.',
            'The containers pipeline adds Trivy vulnerability scanning, SBOM generation, and pinned base images; the runtime image strips the npm toolchain and upgrades OS packages to patch CVE-2026-73566.',
          ),
          feature(
            currentLang,
            'testTube',
            'Nightly benchmark, Firefox E2E & sửa bypass SSRF IPv4-mapped IPv6',
            'Nightly benchmarks, Firefox E2E & the IPv4-mapped IPv6 SSRF fix',
            'Thêm benchmark chạy nightly, ma trận Playwright mở rộng sang Firefox, property-based fuzzing cho guard; đồng thời phát hiện và vá bypass SSRF qua địa chỉ IPv4-mapped IPv6.',
            'Added nightly benchmarks, an expanded Playwright matrix including Firefox, and property-based guard fuzzing; also discovered and fixed an SSRF bypass via IPv4-mapped IPv6 addresses.',
          ),
          feature(
            currentLang,
            'globe',
            'i18n cho thông báo bảo mật server & sửa chuẩn React',
            'Localized server security messages & React correctness fixes',
            'Thông báo lỗi bảo mật phía server được bản địa hóa hoặc chuẩn hóa, các chuỗi tiếng Việt cứng trong AI chat UI được đưa vào i18n, cùng các bản sửa QueryResult typing và React purity violations.',
            'Server-side security messages are localized or normalized, hardcoded Vietnamese strings in the AI chat UI moved into i18n, alongside QueryResult typing and React purity-violation fixes.',
          ),
        ],
      },
      {
        ...
        title: pickLocalizedText(
          currentLang,
          'Provider AI hardened, billing race-safe & DataGrid an toàn production',
          'Hardened AI Providers, Race-safe Billing & Production-safe DataGrid',
        ),
        features: [
          feature(
            currentLang,
            'settings',
            'Custom AI provider lưu được, xác minh được và form thiết kế lại',
            'Custom AI providers persist, verify, and get a redesigned form',
            'Provider OpenAI-compatible do người dùng tự thêm giờ được lưu bền vững, xác minh bằng cách gọi thử endpoint trước khi dùng, và màn hình cấu hình được thiết kế lại với validation chặt hơn cho từng trường.',
            'User-added OpenAI-compatible providers now persist reliably, verify against the endpoint before use, and the configuration screen was redesigned with stricter per-field validation.',
          ),
          feature(
            currentLang,
            'sparkles',
            'PDF gửi native tới provider & migration sang Google GenAI SDK',
            'Native PDF delivery & Google GenAI SDK migration',
            'Tài liệu PDF đính kèm được chuyển thẳng tới provider hỗ trợ thay vì trích xuất văn bản, đồng thời toàn bộ lane Gemini được chuyển sang Google GenAI SDK mới để đồng bộ phiên bản driver.',
            'Attached PDF documents are passed through natively to capable providers instead of text extraction, and the entire Gemini lane migrated to the new Google GenAI SDK for driver-version alignment.',
          ),
          feature(
            currentLang,
            'globe',
            'Khả năng model hiển thị rõ & preflight trước khi chạy',
            'Surfaced model capabilities & preflight checks',
            'Catalog model cho biết năng lực thực (vision, PDF, live search) khớp với provider đang cấu hình, availability được preflight trước request, và lời giải thích AI có cơ chế sửa query an toàn thay vì chạy mù.',
            'The model catalog now reflects real capabilities (vision, PDF, live search) matching the configured provider, availability is preflighted before requests, and AI explanations offer a safe query-correction handoff instead of blind execution.',
          ),
          feature(
            currentLang,
            'shieldCheck',
            'Auth hardened & transaction nguyên tử cho multi-statement',
            'Hardened auth & atomic multi-statement transactions',
            'Token refresh và OAuth identity linking được siết lại chống race condition, billing activation trở nên race-safe, và các script SQL nhiều câu chạy trong một transaction duy nhất — lỗi giữa chừng sẽ rollback toàn bộ.',
            'Token refresh and OAuth identity linking were tightened against race conditions, billing activation became race-safe, and multi-statement SQL scripts now run inside a single transaction — a mid-script failure rolls everything back.',
          ),
          feature(
            currentLang,
            'zap',
            'DataGrid CSV production-safe & error boundary phục hồi',
            'Production-safe CSV DataGrid & recoverable error boundary',
            'Import/export CSV xử lý đúng encoding, delimiter và dữ liệu lớn; ứng dụng có error boundary cho phép phục hồi sau lỗi render thay vì màn hình trắng; dashboard giữ snapshot cũ khi refresh thất bại.',
            'CSV import/export now handles encoding, delimiters, and large payloads correctly; the app gained a recoverable error boundary instead of a white screen; dashboards preserve the previous snapshot when a refresh fails.',
          ),
        ],
      },
      {
