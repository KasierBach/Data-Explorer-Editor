    ...
    return (
        ...
        >
            <FeatureGrid columns={3}>
                <InfoCard icon={<Image className="w-6 h-6 text-blue-500" />} title={t ? 'Ảnh là context thật' : 'Images are real context'} color="blue">
                    <p>
                        {t
                ...
                <InfoCard icon={<Eye className="w-6 h-6 text-emerald-500" />} title={t ? 'Đầu ra không chỉ là mô tả ảnh' : 'The output is not just image description'} color="emerald">
                    <p>
                        {t
                ...
                <InfoCard icon={<Sparkles className="w-6 h-6 text-purple-500" />} title={t ? 'Provider phải đúng capability' : 'The provider must match the capability'} color="purple">
                    <p>
                        {t
            ...
            <DocSection title={t ? 'Workflow vision nên dùng như thế nào?' : 'What a good vision workflow looks like'}>
                <div className="space-y-4">
                    {[
                        {
                            step: '1',
                            title: t ? 'Chọn đúng workspace trước' : 'Start in the right workspace',
                            desc: t
                        ...
                        {
                            step: '2',
                            title: t ? 'Đính kèm ảnh và nói rõ mục tiêu' : 'Attach the image and state the goal clearly',
                            desc: t
                        ...
                        {
                            step: '3',
                            title: t ? 'Review output rồi mới chạy' : 'Review the output before running anything',
                            desc: t
                        ...
                        {
                            step: '4',
                            title: t ? 'Dùng ảnh như điểm khởi đầu, không phải chân lý tuyệt đối' : 'Treat the image as a starting point, not absolute truth',
                            desc: t
                        ...
                        <div key={item.step} className="flex gap-4 items-start p-5 border rounded-2xl bg-muted/10">
            ...
            <DocSection title={t ? 'Provider vision nào đang đáng tin trong repo hiện tại?' : 'Which vision providers are trustworthy in the current repo?'}>
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-border/60 bg-card/40 p-5">
                        <h3 className="text-sm font-bold">TokenRouter / MiniMax-M3</h3>
                        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                            {t
                                ? 'Hiện đã được repo cho phép đi theo đường vision khi bạn explicit chọn `tokenrouter:MiniMax-M3`. Tuy nhiên lane này không nên bị coi là live-search lane.'
                                : 'This lane is now allowed through the vision path when you explicitly choose `tokenrouter:MiniMax-M3`. However, it should not be treated as a live-search lane.'}
                        </p>
                    </div>
                </div>
            </DocSection>

            <DocSection title={t ? 'Vision không làm thay phần nào của con người?' : 'What vision does not replace'}>
                <div className="grid gap-4 md:grid-cols-3">
                    <div className="rounded-2xl border border-border/60 bg-card/40 p-5">
                        <h3 className="text-sm font-bold">{t ? 'Không thay review query' : 'It does not replace query review'}</h3>
                        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                            {t
                    ...
                    <div className="rounded-2xl border border-border/60 bg-card/40 p-5">
                        <h3 className="text-sm font-bold">{t ? 'Không tự biến thành live web search' : 'It does not become live web search automatically'}</h3>
                        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                            {t
                    ...
                    <div className="rounded-2xl border border-border/60 bg-card/40 p-5">
                        <h3 className="text-sm font-bold">{t ? 'Không biến ảnh thành sự thật tuyệt đối' : 'It does not turn an image into absolute truth'}</h3>
                        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                            {t
            ...
            <DocSection title={t ? 'Loại ảnh nào phù hợp nhất?' : 'Which image types work best?'}>
                <div className="grid gap-4 md:grid-cols-3">
                    {[
                        {
                        ...
                        {
                            title: t ? 'Whiteboard, ghi chú, phác thảo' : 'Whiteboards, notes, and sketches',
                            desc: t
            ...
            <Callout type="warning">
                <p className="font-bold">{t ? 'Cần lane có vision thật' : 'A true vision lane is required'}</p>
                <p className="mt-2 text-sm text-muted-foreground">
                    {t
                        ? 'Để dùng ảnh đính kèm trong trạng thái wiring hiện tại, bạn cần ít nhất một trong các cấu hình sau: `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, hoặc `TOKENROUTER_API_KEY` khi muốn dùng explicit `MiniMax-M3`. Nếu không có lane vision-capable, app sẽ từ chối tác vụ ảnh.'
                        : 'To use image attachments with the current wiring, you need at least one of: `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, or `TOKENROUTER_API_KEY` when you want explicit `MiniMax-M3`. Without a vision-capable lane, the app rejects image-based tasks.'}
                </p>
            </Callout>
