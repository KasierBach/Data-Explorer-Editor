...
export function PrerequisitesSection({ lang }: Props) {
  ...
  return (
    ...
    >
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        <div className="p-6 border rounded-3xl bg-blue-500/5 border-blue-500/10">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {t
              ? "Node.js 20.x LTS được khuyên dùng. Frontend build bằng Vite 7.x."
              : "Node.js 20.x LTS is recommended. The frontend builds with Vite 7.x."}
          </p>
        </div>
        <div className="p-6 border rounded-3xl bg-emerald-500/5 border-emerald-500/10">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {t
              ? "Một PostgreSQL riêng cho metadata app và một Redis instance cho cache, queue, notifications, presence, và throttling."
              : "A dedicated PostgreSQL database for app metadata plus a Redis instance for cache, queues, notifications, presence, and throttling."}
          </p>
        </div>
        <div className="p-6 border rounded-3xl bg-violet-500/5 border-violet-500/10">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {t
              ? "JWT secret mạnh, ENCRYPTION_KEY đúng 32 ký tự, và outbound HTTPS tới AI providers nếu bạn bật AI routing."
              : "A strong JWT secret, an ENCRYPTION_KEY with exactly 32 characters, and outbound HTTPS to AI providers if you enable AI routing."}
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <FeatureGrid>
            ...
            >
              <p className="text-xs">
                {t
                  ? "Khuyên dùng Node.js 20.x LTS cho dev và production builds."
                  : "Node.js 20.x LTS is recommended for development and production builds."}
              </p>
            </InfoCard>
            <InfoCard
            ...
            >
              <p className="text-xs">
                {t
                  ? "npm là đường được dùng nhiều nhất trong repo hiện tại. Tránh trộn nhiều package manager để khỏi lệch lockfile."
                  : "npm is the most common path in the current repo. Avoid mixing package managers to prevent lockfile drift."}
              </p>
            </InfoCard>
            <InfoCard
      ...
      <DocSection
        title={
          t ? "Ma trận tương thích database" : "Database compatibility matrix"
        }
      >
              ...
              {[
                {
                  ...
                  ver: "12+",
                  feat: t
                    ? "SQL workspace chính, health checks, explain, dashboards."
                    : "Primary SQL workspace, health checks, explain, dashboards.",
                },
                {
                ...
                },
                {
                  ...
                  ver: "2017+",
                  feat: t
                    ? "SQL workspace, metadata explorer, AI-assisted query flow."
                    : "SQL workspace, metadata explorer, AI-assisted query flow.",
                },
                {
                  engine: "MongoDB",
                  ver: "6+",
                  feat: t
                    ? "NoSQL workspace và document exploration."
                    : "NoSQL workspace and document exploration.",
                },
                {
                  engine: "MongoDB Atlas (SRV)",
                  ver: "Current",
                  feat: t
                    ? "Atlas SRV parsing, SSRF-aware validation, NoSQL workspace."
                    : "Atlas SRV parsing, SSRF-aware validation, NoSQL workspace.",
                },
              ].map((row, i) => (
          ...
          {[
            {
              ...
              desc: t
                ? "Backend cần outbound HTTPS (443) tới provider bạn bật: Gemini, OpenRouter, Groq, Cerebras, hoặc Beeknoee. Nếu key thiếu hoặc provider không reach được, lane đó sẽ bị loại khỏi chuỗi chạy thay vì được giả vờ là đang sẵn sàng."
                : "The backend needs outbound HTTPS (443) to whichever providers you enable: Gemini, OpenRouter, Groq, Cerebras, or Beeknoee. If a key is missing or the provider is unreachable, that lane is removed from the execution chain rather than treated as if it were available.",
            },
            {
            ...
            <li
              key={i}
              className="flex gap-4 p-5 border rounded-2xl items-start bg-muted/5"
            >
