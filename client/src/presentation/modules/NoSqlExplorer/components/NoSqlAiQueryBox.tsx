...
import { NOSQL_STAGE_TYPES } from "../aggregationBuilderUtils";

interface NoSqlAiQueryBoxProps {
  onGenerate: (mql: string) => boolean | void;
...
export const NoSqlAiQueryBox: React.FC<NoSqlAiQueryBoxProps> = ({
  ...
  const resolvedNoSql = resolveAiSelection(
    preferences.nosqlModel,
    assistantSelection,
  ...
  const commandLabel = aggregateOnly
    ? lang === "vi"
      ? "pipeline"
      : "pipeline"
    : text.noSqlAi.mqlCommandLabel;
  const generatorLabel = aggregateOnly
    ? lang === "vi"
      ? "Tạo pipeline bằng AI"
      : "Build a pipeline with AI"
    : text.noSqlAi.mqlGeneratorTitle;
  const collapsedPrompt = aggregateOnly
    ? lang === "vi"
      ? "Mô tả kết quả bạn muốn nhận"
      : "Describe the result you need"
    : text.noSqlAi.collapsedPromptMongo;
  const expandedPlaceholder = aggregateOnly
    ? lang === "vi"
      ? "Ví dụ: Nhóm phim theo thể loại, đếm số lượng và lấy 10 nhóm nhiều nhất..."
      : "Example: Group movies by genre, count them, and return the top 10 groups..."
    : text.noSqlAi.expandedPlaceholderMongo;
  const capabilityTags = aggregateOnly
    ? ["$match", "$group", "$project", "$sort", "$limit"]
    : isReadOnly
      ? ["find", "aggregate"]
      : ["find", "aggregate", "update", "delete"];
  const capabilityNote = aggregateOnly
    ? lang === "vi"
      ? "AI chỉ tạo aggregation pipeline cho collection hiện tại. Bạn vẫn có thể xem và chỉnh từng stage trước khi chạy."
      : "AI only creates an aggregation pipeline for the current collection. You can review and edit every stage before running it."
    : isReadOnly
      ? text.noSqlAi.capabilityNoteReadOnly
      : text.noSqlAi.capabilityNoteMutable;
  const runHotkeyLabel =
    navigator.platform.toUpperCase().indexOf("MAC") >= 0
      ? text.noSqlAi.runHotkey.replace("CTRL", "CMD")
      : text.noSqlAi.runHotkey;
  const quickPrompts: QuickPrompt[] = aggregateOnly
    ? lang === "vi"
      ? [
          {
            label: "Top nhóm",
            prompt:
              "Nhóm theo một field phù hợp, đếm số lượng, sắp xếp giảm dần và lấy top 10.",
          },
          {
            label: "Lọc và thống kê",
            prompt:
              "Lọc các document hợp lệ rồi thống kê theo field quan trọng nhất.",
          },
          {
            label: "Dữ liệu gần đây",
            prompt:
              "Lấy dữ liệu gần đây nhất, sắp xếp theo thời gian giảm dần và giới hạn 20 kết quả.",
          },
        ]
      : [
          {
            label: "Top groups",
            prompt:
              "Group by a relevant field, count documents, sort descending, and return the top 10.",
          },
          {
            label: "Filter and summarize",
            prompt:
              "Filter valid documents, then summarize them by the most relevant field.",
          },
          {
            label: "Recent data",
            prompt:
              "Return the most recent documents, sorted by time descending and limited to 20 results.",
          },
        ]
    : isReadOnly
      ? [
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await apiService.post<{
        ...
        prompt: aggregateOnly
          ? `For collection "${collectionName || "current collection"}", return only one JSON MongoDB command with action "aggregate" and a valid pipeline array. Use only these supported stages: ${NOSQL_STAGE_TYPES.join(", ")}. User request: ${query}`
          : collectionName
      ...
      const explicitMutationIntent =
        /\b(insert|update|delete|remove|drop|truncate|set|unset|inc|push|pull|expire|persist|rename|flush)\b/i.test(
          query,
        );
      const isMutatingCommand =
        !!mutationAction && MUTATING_MONGO_ACTIONS.has(mutationAction);
      const isTooLong =
        generatedCommand.length > 1200 ||
        generatedCommand.split("\n").length > 20;

      if (aggregateOnly && generatedCommand && mutationAction !== "aggregate") {
        const aggregateError =
          lang === "vi"
            ? "AI chưa trả về aggregation pipeline hợp lệ. Hãy mô tả lại mục tiêu hoặc thử model khác."
            : "AI did not return a valid aggregation pipeline. Rephrase the goal or try another model.";
        setExplanation(aggregateError);
        toast.error(aggregateError);
      } else if (
        !aggregateOnly &&
        generatedCommand &&
        !explicitMutationIntent &&
        (isMutatingCommand || isTooLong)
      ) {
        ...
        const wasAccepted = onGenerate(generatedCommand);
        if (wasAccepted === false) {
          const unsupportedError =
            lang === "vi"
              ? "Pipeline AI chưa hợp lệ hoặc có stage chưa được trình dựng hỗ trợ. Hãy thử mô tả rõ hơn hoặc chỉnh trong editor."
              : "The AI pipeline is invalid or contains an unsupported stage. Clarify the request or edit it manually.";
          setExplanation(unsupportedError);
  ...
  return (
    <div
      className={cn(
        ...
        isExpanded
          ? "bg-background/80 shadow-2xl ring-1 ring-green-500/20"
        ...
        {!isExpanded ? (
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <span className="font-medium opacity-70">{collapsedPrompt}</span>
            </div>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-1.5 opacity-40 hover:opacity-100 transition-opacity cursor-help">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest leading-none">
                    ...
                    {collectionName ||
                      currentDatabase ||
                      text.noSqlAi.defaultCollection}
                  </span>
                </div>
              </div>
              <div className="flex items-center space-x-2">
