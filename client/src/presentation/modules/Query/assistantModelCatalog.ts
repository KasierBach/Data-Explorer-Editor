...
const BUILT_IN_MODEL_GROUPS: AssistantModelGroup[] = [
  {
    ...
    items: [
      ...
      { id: 'gemini-3.1-flash-lite', label: 'Gemini 3.1 Flash Lite (Fast)', isNew: true },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'deepseek/deepseek-v4-flash:free', label: 'DeepSeek V4 Flash', isNew: true },
...
export function getAssistantModelCatalog(customProviders: CustomAiProvider[] = []): AssistantModelGroup[] {
...
export function findAssistantModelLabel(modelId: string, customProviders: CustomAiProvider[] = []) {
  const groups = getAssistantModelCatalog(customProviders);
  return groups.flatMap((group) => group.items).find((item) => item.id === modelId)?.label ?? modelId;
}
