        ...
        <div className="h-14 border-b flex items-center px-4 bg-card justify-between select-none shrink-0 sticky top-0 z-[60]">
            ...
            <div className="flex items-center gap-2 md:gap-6">
                ...
                {isActualMobile ? (
                    <div className="flex bg-muted p-1 rounded-md border items-center">
                        <Button 
                            variant={isSqlRoute ? 'secondary' : 'ghost'} 
                            size="sm" 
                            className="h-7 px-2 text-xs min-w-[44px] flex gap-1.5"
                            onClick={() => navigate('/sql-explorer')}
                        >
                            <Database className="w-3.5 h-3.5" />
                            {!isSmallMobile && <span className="font-semibold">SQL</span>}
                        </Button>
                        <Button 
                            variant={isNoSqlRoute ? 'secondary' : 'ghost'} 
                            size="sm" 
                            className="h-7 px-2 text-xs min-w-[44px] flex gap-1.5 text-green-600 dark:text-green-400"
                            onClick={() => navigate('/nosql-explorer')}
                        >
                            <Leaf className="w-3.5 h-3.5" />
                            {!isSmallMobile && <span className="font-semibold text-green-700 dark:text-green-400">NoSQL</span>}
                        </Button>
                    </div>
                ) : (
                    <div className="flex bg-muted p-1 rounded-md border items-center">
            ...
            <div className="flex items-center gap-2 md:gap-3">
                ...
                <NavUserSection 
                    ...
                    isDesktopModeOnMobile={isDesktopModeOnMobile}
                    toggleDesktopModeOnMobile={toggleDesktopModeOnMobile}
                    lang={lang}
